...
function initBuffers() {
	...
	var dim_x = document.getElementById("dim_ox").value;
	var dim_y = document.getElementById("dim_oy").value;
	var dim_z = document.getElementById("dim_oz").value;
	if(dim_x=="") {dim_x=0.01;}
	if(dim_y=="") {dim_y=0.01;}
	if(dim_z=="") {dim_z=0.01;}
	
	vertices = drawPlate(dim_x, dim_y, dim_z);
...
function webGLStart() {
	var canvas = document.getElementById("modeling-canvas");
	initGL(canvas);
	initShaders();
	initBuffers();
...
function drawPlate(dim_x, dim_y, dim_z) {
	var plate = [
		// Front face
		-1.0*dim_x, -1.0*dim_y, 1.0*dim_z,
		 1.0*dim_x, -1.0*dim_y, 1.0*dim_z,
		 1.0*dim_x, 1.0*dim_y, 1.0*dim_z, 
		-1.0*dim_x, 1.0*dim_y, 1.0*dim_z,
		// Back face
